import { createContext, useContext, useMemo, useState } from "react";

type Language = "en" | "ru";
type DictValue = string | Record<string, DictValue>;

const STORAGE_KEY = "labsync-lang";

const resources: Record<Language, Record<string, DictValue>> = {
  en: {
    common: {
      language: "Language",
      english: "English",
      russian: "Russian",
      board: "Board",
      samples: "Samples",
      actions: "Actions",
      admin: "Admin",
      settings: "Settings",
      warehouse: "Warehouse",
      labOperator: "Lab Operator",
      actionSupervision: "Action Supervision",
      guest: "Guest",
    },
    commonShort: {
      warehouse: "Warehouse",
      labOperator: "Lab Operator",
      actionSupervision: "Action",
      admin: "Admin",
    },
    methods: {
      sara: "SARA",
      ir: "IR",
      mass_spectrometry: "Mass Spectrometry",
      viscosity: "Rheology",
      electrophoresis: "Electrophoresis",
      nmr: "NMR",
    },
    methodsShort: {
      sara: "SARA",
      ir: "IR",
      mass_spectrometry: "Mass Spectr.",
      viscosity: "Rheology",
      electrophoresis: "Electrophoresis",
      nmr: "NMR",
    },
    topBar: {
      searchPlaceholder: "Search samples, analyses, or IDs...",
      searchAria: "Search samples and analyses",
      rolePlaceholder: "Role",
      toggleTheme: "Toggle theme",
      notifications: "Notifications",
      markAllRead: "Mark all as read",
      markAllReadShort: "Mark all read",
      noNotifications: "No new notifications.",
      signIn: "Sign in",
      logout: "Sign out",
    },
    navigation: {
      backToTop: "Go to top",
    },
    login: {
      signIn: "Sign in",
      subtitle: "Enter your credentials to continue.",
      username: "Username",
      password: "Password",
      rememberMe: "Remember me",
      forgotPassword: "Forgot password?",
      signingIn: "Signing in...",
      hidePassword: "Hide password",
      showPassword: "Show password",
      sessionHint: "Session will end when you sign out or clear browser data.",
      requestSubtitle: "Enter your username and account email. We will send a password reset token.",
      email: "Email",
      sending: "Sending...",
      sendResetToken: "Send reset token",
      backToSignIn: "Back to sign in",
      confirmSubtitle: "Enter reset token and set a new password.",
      resetToken: "Reset token",
      newPassword: "New password",
      confirmNewPassword: "Confirm new password",
      resetting: "Resetting...",
      resetPassword: "Reset password",
      mustChangeSubtitle: "You must set a new password before continuing.",
      currentPassword: "Current password",
      confirmPassword: "Confirm new password",
      updating: "Updating...",
      updatePassword: "Update password",
      errors: {
        loginFailed: "Login failed",
        currentPasswordRequired: "Current password is required.",
        newPasswordMin: "New password must be at least 8 characters.",
        passwordMismatch: "New password and confirmation do not match.",
        passwordChangeFailed: "Password change failed",
        usernameRequired: "Username is required.",
        emailRequired: "Email is required.",
        emailInvalid: "Please enter a valid email address.",
        requestFailed: "Failed to request password reset.",
        requestFailedWithStatus: "Request failed: {{status}}",
        resetTokenRequired: "Reset token is required.",
        resetFailed: "Failed to reset password.",
        resetFailedWithStatus: "Reset failed: {{status}}",
      },
      messages: {
        requestSuccess: "If that email exists, a reset email has been sent.",
        resetSuccess: "Password reset completed. You can now sign in with your new password.",
      },
    },
    admin: {
      toast: {
        failedLoadUsers: "Failed to load users",
        failedLoadEventLog: "Failed to load event log",
        usernameRequired: "Username required",
        fullNameRequired: "Full name required",
        defaultRoleRequired: "Default role required",
        emailRequired: "Email required",
        invalidEmail: "Enter a valid email address",
        userCreated: "User created",
        defaultPassword: "Default password: {{password}}",
        failedCreateUser: "Failed to create user",
        failedDeleteUser: "Failed to delete user",
        usernameUpdated: "Username updated",
        failedUpdateUsername: "Failed to update username",
        fullNameUpdated: "Full name updated",
        failedUpdateFullName: "Failed to update full name",
        emailUpdated: "Email updated",
        failedUpdateEmail: "Failed to update email",
        backendUnreachable: "Backend unreachable",
      },
      page: {
        usersRolesTitle: "Users & roles",
        usersRolesSubtitle: "Create users, edit roles, or delete accounts.",
        fullName: "Full name",
        defaultRole: "Default role",
        creating: "Creating...",
        createUser: "Create user",
        defaultPasswordForNewUsers: "Default password for new users:",
        roles: "Roles",
        actions: "Actions",
        noRolesYet: "No roles yet",
        selectRoles: "Select roles",
        noRolesFound: "No roles found.",
        noMethodPermissions: "No method permissions",
        methodPermissions: "Method permissions",
        noMethodsFound: "No methods found.",
        loadingUsers: "Loading users...",
        noUsersFound: "No users found.",
        eventLogTitle: "Event log",
        eventLogSubtitle: "Review workflow activity with filters, sorting, and search.",
        searchEventsPlaceholder: "Search details, actor, action...",
        allEntities: "All entities",
        allActions: "All actions",
        newestFirst: "Newest first",
        oldestFirst: "Oldest first",
        applyFilters: "Apply filters",
        loading: "Loading...",
        showingCount: "Showing {{shown}} of {{total}}",
        collapseEventLog: "Collapse event log to top 10 entries",
        expandEventLog: "Expand event log to full list",
        showTop10: "Show top 10",
        showFullList: "Show full list",
        timestamp: "Timestamp",
        actor: "Actor",
        entity: "Entity",
        action: "Action",
        details: "Details",
        system: "System",
        loadingEvents: "Loading events...",
        noEventsForFilters: "No events found for current filters.",
        placeholders: {
          username: "e.g. lab.tech",
          fullName: "e.g. Ivan Petrov",
          email: "e.g. user@company.com",
        },
        aria: {
          changeUsername: "Change username for {{username}}",
          changeFullName: "Change full name for {{username}}",
          changeEmail: "Change email for {{username}}",
        },
        event: {
          entities: {
            sample: "sample",
            planned_analysis: "planned_analysis",
            conflict: "conflict",
            user: "user",
          },
          actions: {
            created: "created",
            updated: "updated",
            delete: "delete",
            deleted: "deleted",
            status_change: "status_change",
            operator_assigned: "operator_assigned",
            operator_unassigned: "operator_unassigned",
            role_changed: "role_changed",
            method_permissions_changed: "method_permissions_changed",
            password_reset_requested: "password_reset_requested",
            password_reset_completed: "password_reset_completed",
            password_changed: "password_changed",
          },
          details: {
            self_service: "Self service",
            email_flow: "Email flow",
            keys: {
              username: "Username",
              email: "Email",
              roles: "Roles",
              methods: "Method permissions",
              status: "Status",
              well_id: "Well ID",
              horizon: "Horizon",
              sampling_date: "Sampling date",
              arrival_date: "Arrival date",
              storage_location: "Storage location",
              assigned_to: "Assigned to",
              sample: "Sample",
              method: "Method",
              assignees: "Assignees",
              target: "Target",
              resolution_note: "Resolution note",
            },
          },
        },
        dialogs: {
          to: "to",
          confirm: "Confirm",
          updateUsername: {
            title: "Update username",
            description: "Change the username for",
            save: "Save username",
          },
          confirmUsername: {
            title: "Confirm username change",
            description: "Update username from",
          },
          updateFullName: {
            title: "Update full name",
            description: "Change the full name for",
            save: "Save full name",
          },
          confirmFullName: {
            title: "Confirm full name change",
            description: "Update full name from",
          },
          updateEmail: {
            title: "Update user email",
            description: "Change the email for",
            save: "Save email",
          },
          confirmEmail: {
            title: "Confirm email change",
            description: "Update",
          },
        },
      },
    },
    settings: {
      title: "Settings",
      systemStatus: "System status",
      subtitle: "Frontend still uses mock data; backend is used for health only.",
      backendReachable: "Backend reachable",
      unexpectedResponse: "Unexpected response",
      failedBackend: "Failed to reach backend",
      checking: "Checking...",
      healthy: "Healthy",
      unavailable: "Unavailable",
      idle: "Idle",
      retry: "Retry",
    },
    samplesPage: {
      title: "Sample registry",
      subtitle: "All unique samples and analysis statuses.",
      loading: "Loading samples...",
      empty: "No samples yet.",
      common: {
        reset: "Reset",
        cancel: "Cancel",
        apply: "Apply",
      },
      filters: {
        sampleId: "Filter Sample ID",
        wellId: "Filter Well ID",
        samplingDate: "Filter sampling date",
        arrivalDate: "Filter arrival date",
        operator: "Filter operator",
      },
      methods: {
        button: "Methods",
        title: "Methods filter",
        chooseMethod: "Choose method",
        any: "Any",
        done: "Done",
        notDone: "Not done",
      },
      sort: {
        button: "Sort",
        order: "Sort order",
        by: "Sort by",
        chooseField: "Choose field",
        ascending: "Ascending",
        descending: "Descending",
      },
      columns: {
        sampleId: "Sample ID",
        wellId: "Well ID",
        ngdu: "NGDU",
        wellNumber: "Well number",
        shop: "Shop",
        field: "Field",
        injectionWell: "Injection well",
        horizon: "Horizon",
        samplingDate: "Sampling date",
        arrivalDate: "Arrival date",
        storageLocation: "Storage location",
        warehouseStatus: "Warehouse status",
        labStatus: "Lab status",
        adminStatus: "Admin status",
        analyses: "Analyses",
        method: "Method",
        done: "Done",
        operators: "Operators",
      },
      values: {
        yes: "Yes",
        no: "No",
        na: "N/A",
        unassigned: "Unassigned",
      },
    },
    actionsPage: {
      title: "Actions registry",
      subtitle: "Mock technological actions table.",
      common: {
        reset: "Reset",
        cancel: "Cancel",
        apply: "Apply",
      },
      filters: {
        wellId: "Filter Well ID",
        actionId: "Filter Action ID",
        startDate: "Filter start date",
        endDate: "Filter end date",
        actionType: "Filter action type",
      },
      sort: {
        button: "Sort",
        order: "Sort order",
        by: "Sort by",
        chooseField: "Choose field",
        ascending: "Ascending",
        descending: "Descending",
      },
      columns: {
        wellId: "Well ID",
        actionId: "Action ID",
        startDate: "Start date",
        endDate: "End date",
        success: "Success",
        actionType: "Action type",
        incrementalOil: "Actual incremental oil production investment year, t",
        avgDailyRate: "Actual average daily production rate before the action, t/day",
      },
    },
    board: {
      roleView: {
        warehouse_worker: "Warehouse view: samples and storage",
        lab_operator: "Lab view: analyses",
        action_supervision: "Action supervision view",
        admin: "Admin view",
      },
      sampleTrackingBoard: "Sample Tracking Board",
      samplesAcrossStages: "{{count}} samples across {{stages}} stages",
      methods: "Methods",
      filterVisibility: "Filter visibility",
      changesSaved: "Changes saved",
      filterVisibilityUpdated: "Filter visibility updated.",
      failedUpdateFilters: "Failed to update filters",
      failedLoadData: "Failed to load data",
      sort: "Sort",
      sortOrder: "Sort order",
      reset: "Reset",
      sortBy: "Sort by",
      chooseField: "Choose field",
      sampleId: "Sample ID",
      samplingDate: "Sampling date",
      methodsDone: "Methods Done",
      ascending: "Ascending",
      descending: "Descending",
      cancel: "Cancel",
      apply: "Apply",
      showAssignedOnly: "Show only assigned to me",
      showIncompleteOnly: "Show only incomplete",
      undo: "Undo",
      syncing: "Syncing...",
      refresh: "Refresh",
      addConflict: "Add conflict",
      newSample: "New Sample",
      newSampleDialog: {
        title: "Create new sample",
        description: "Minimal fields for a sample card. Saved locally only.",
        sampleId: "Sample ID",
        wellId: "Well ID",
        horizon: "Horizon",
        samplingDate: "Sampling date",
        arrivalDate: "Arrival date",
        pickDate: "Pick a date",
        storageLocation: "Storage location",
        create: "Create",
        errors: {
          required: "All required fields must be filled",
          futureDate: "Sampling date cannot be in the future",
          futureArrivalDate: "Arrival date cannot be in the future",
          arrivalBeforeSampling: "Arrival date cannot be before sampling date",
          duplicateSampleId: "Sample ID already exists",
          fillStorageParts: "Fill Fridge, Bin, and Place",
          invalidStorageFormat: "Storage location must be \"Fridge {A1} · Bin {B2} · Place {C3}\"",
        },
      },
      notifications: {
        methodAssignmentTitle: "Method assignment",
        methodUnassignmentTitle: "Method unassignment",
        methodAssigned: "{{method}} on {{sampleId}} assigned to you.",
        methodAssignedBy: "{{method}} on {{sampleId}} assigned to you by {{actor}}.",
        methodUnassigned: "{{method}} on {{sampleId}} unassigned from you.",
        methodUnassignedBy: "{{method}} on {{sampleId}} unassigned from you by {{actor}}.",
        newPlannedSampleTitle: "New planned sample",
        newPlannedSampleDesc: "{{sampleId}} appeared in Planned without using \"New sample\".",
        returnedForAnalysisTitle: "Returned for analysis",
        returnedToWarehouseDesc: "{{sampleId}} was returned to Warehouse by Admin.",
        uploadedBatchTitle: "Uploaded batch",
        uploadedBatchDesc: "{{sampleId}} is in Uploaded batch.",
        conflictDetectedTitle: "Conflict detected",
        conflictDetectedDesc: "{{sampleId}} is in Conflicts.",
        issueReportedTitle: "Issue reported",
        issueReportedDesc: "{{sampleId}} is in Issues.",
        needsAttentionTitle: "Needs attention",
        needsAttentionDesc: "{{sampleId}} is in Needs attention.",
        newPlannedAnalysisTitle: "New planned analysis",
        newPlannedAnalysisDesc: "{{sampleId}} is in Planned.",
        returnedToLabDesc: "{{sampleId}} was returned to Lab by Admin.",
      },
      loadingBoard: "Loading board...",
      noItemsYet: "No items yet. Create a sample or analysis to get started.",
      columns: {
        planned: "Planned",
        awaiting_arrival: "Awaiting arrival",
        stored: "Stored",
        issues: "Issues",
        in_progress: "In progress",
        needs_attention: "Needs attention",
        completed: "Completed",
        uploaded_batch: "Uploaded batch",
        conflicts: "Conflicts",
        deleted: "Deleted",
      },
      columnsShort: {
        planned: "Planned",
        awaiting_arrival: "Awaiting",
        in_progress: "In progress",
        needs_attention: "Attention",
        completed: "Completed",
        stored: "Stored",
        issues: "Issues",
        conflicts: "Conflicts",
        uploaded_batch: "Batch",
        deleted: "Deleted",
      },
      card: {
        sample: "Sample",
        analysis: "Analysis",
        conflictStatus: "Conflict status",
        uploadedBatch: "Uploaded batch",
        batch: "Batch",
        conflict: "Conflict",
        conflicts: "Conflicts",
        resolved: "Resolved",
        stored: "Stored",
        sampling: "Sampling",
        arrival: "Arrival",
        well: "Well",
        horizon: "Horizon",
        reason: "Reason",
        returnNote: "Return note",
        issue: "Issue",
        fridge: "Fridge",
        bin: "Bin",
        place: "Place",
        done: "Done",
        delete: "Delete",
        restore: "Restore",
        storedAsNotResolved: "Stored as not-resolved",
        returnForAnalysis: "Return for analysis",
        failed: "Failed",
        unassigned: "Unassigned",
      },
      cardShort: {
        well: "Well",
        horizon: "Horiz.",
        fridge: "Fridge",
        bin: "Bin",
        place: "Place",
        unassigned: "Unassigned",
      },
      actionsShort: {
        sort: "Sort",
        filterVisibility: "Filters",
        applyFilters: "Apply",
        resolveConflict: "Resolve",
        storedAsNotResolved: "Not resolved",
        returnForAnalysis: "Return",
      },
    },
    panel: {
      sampleStatus: "Sample status",
      analysisStatus: "Analysis status",
      latestIssue: "Latest issue",
      latestReturnNote: "Latest return note",
      timeUnavailable: "Time unavailable",
      assignedTo: "Assigned to",
      addAssignee: "Add assignee",
      samplingDate: "Sampling date",
      arrivalDate: "Arrival date",
      storage: "Storage",
      resolutionNote: "Resolution note",
      systemNote: "System note",
      issueAndReturnNotes: "Issue & Return Notes",
      issue: "Issue",
      returnNote: "Return note",
      comments: "Comments",
      noCommentsYet: "No comments yet.",
      commentAs: "Comment as",
      unknown: "Unknown",
      writeComment: "Write a comment...",
      addComment: "Add comment",
      assignOperatorToMethod: "Assign operator to method",
      selectMethod: "Select method",
      assignToLabOperator: "Assign to lab operator",
      noOperatorsForMethod: "No operators have permission for this method.",
      methodRequired: "Method is required",
      selectOperator: "Select an operator to assign",
      labAssignOnlySelf: "Lab operator can assign only themselves",
      noEligibleOperators: "No eligible operators for this method",
      assignOperator: "Assign operator",
      addAnalysisAdmin: "Add analysis (Admin)",
      analysisPlaceholder: "e.g. NMR",
      assignOptional: "Assign to (optional)",
      analysisTypeRequired: "Analysis type is required",
      addAnalysis: "Add analysis",
      resolveConflict: "Resolve conflict",
      resolutionNoteOptional: "Resolution note",
      markResolved: "Mark resolved",
      old: "OLD",
      new: "NEW",
    },
  },
  ru: {
    common: {
      language: "Язык",
      english: "Английский",
      russian: "Русский",
      board: "Доска",
      samples: "Образцы",
      actions: "Воздействия",
      admin: "Админ",
      settings: "Настройки",
      warehouse: "Склад",
      labOperator: "Лаборант",
      actionSupervision: "Контроль воздействий",
      guest: "Гость",
    },
    commonShort: {
      warehouse: "Склад",
      labOperator: "Лаборант",
      actionSupervision: "Воздействия",
      admin: "Админ",
    },
    methods: {
      sara: "SARA",
      ir: "ИК",
      mass_spectrometry: "Масс-спектрометрия",
      viscosity: "Реология",
      electrophoresis: "Электрофорез",
      nmr: "ЯМР",
    },
    methodsShort: {
      sara: "SARA",
      ir: "ИК",
      mass_spectrometry: "Масс-спектр.",
      viscosity: "Реология",
      electrophoresis: "Электрофорез",
      nmr: "ЯМР",
    },
    topBar: {
      searchPlaceholder: "Поиск образцов, анализов или ID...",
      searchAria: "Поиск образцов и анализов",
      rolePlaceholder: "Роль",
      toggleTheme: "Сменить тему",
      notifications: "Уведомления",
      markAllRead: "Отметить все прочитанными",
      markAllReadShort: "Прочитано всё",
      noNotifications: "Новых уведомлений нет.",
      signIn: "Вход",
      logout: "Выйти",
    },
    navigation: {
      backToTop: "Наверх",
    },
    login: {
      signIn: "Вход",
      subtitle: "Введите учетные данные для продолжения.",
      username: "Имя пользователя",
      password: "Пароль",
      rememberMe: "Запомнить меня",
      forgotPassword: "Забыли пароль?",
      signingIn: "Вход...",
      hidePassword: "Скрыть пароль",
      showPassword: "Показать пароль",
      sessionHint: "Сессия завершится после выхода или очистки данных браузера.",
      requestSubtitle: "Введите имя пользователя и email учетной записи. Мы отправим токен сброса пароля.",
      email: "Эл. почта",
      sending: "Отправка...",
      sendResetToken: "Отправить токен сброса",
      backToSignIn: "Назад ко входу",
      confirmSubtitle: "Введите токен сброса и задайте новый пароль.",
      resetToken: "Токен сброса",
      newPassword: "Новый пароль",
      confirmNewPassword: "Подтвердите новый пароль",
      resetting: "Сброс...",
      resetPassword: "Сбросить пароль",
      mustChangeSubtitle: "Перед продолжением необходимо установить новый пароль.",
      currentPassword: "Текущий пароль",
      confirmPassword: "Подтвердите новый пароль",
      updating: "Обновление...",
      updatePassword: "Обновить пароль",
      errors: {
        loginFailed: "Не удалось войти",
        currentPasswordRequired: "Требуется текущий пароль.",
        newPasswordMin: "Новый пароль должен содержать не менее 8 символов.",
        passwordMismatch: "Новый пароль и подтверждение не совпадают.",
        passwordChangeFailed: "Не удалось изменить пароль",
        usernameRequired: "Требуется имя пользователя.",
        emailRequired: "Требуется email.",
        emailInvalid: "Введите корректный адрес электронной почты.",
        requestFailed: "Не удалось запросить сброс пароля.",
        requestFailedWithStatus: "Ошибка запроса: {{status}}",
        resetTokenRequired: "Требуется токен сброса.",
        resetFailed: "Не удалось сбросить пароль.",
        resetFailedWithStatus: "Ошибка сброса: {{status}}",
      },
      messages: {
        requestSuccess: "Если такой email существует, письмо для сброса пароля отправлено.",
        resetSuccess: "Пароль сброшен. Теперь войдите с новым паролем.",
      },
    },
    admin: {
      toast: {
        failedLoadUsers: "Не удалось загрузить пользователей",
        failedLoadEventLog: "Не удалось загрузить журнал событий",
        usernameRequired: "Требуется имя пользователя",
        fullNameRequired: "Требуется полное имя",
        defaultRoleRequired: "Требуется роль по умолчанию",
        emailRequired: "Требуется email",
        invalidEmail: "Введите корректный email",
        userCreated: "Пользователь создан",
        defaultPassword: "Пароль по умолчанию: {{password}}",
        failedCreateUser: "Не удалось создать пользователя",
        failedDeleteUser: "Не удалось удалить пользователя",
        usernameUpdated: "Имя пользователя обновлено",
        failedUpdateUsername: "Не удалось обновить имя пользователя",
        fullNameUpdated: "Полное имя обновлено",
        failedUpdateFullName: "Не удалось обновить полное имя",
        emailUpdated: "Email обновлен",
        failedUpdateEmail: "Не удалось обновить email",
        backendUnreachable: "Бэкенд недоступен",
      },
      page: {
        usersRolesTitle: "Пользователи и роли",
        usersRolesSubtitle: "Создавайте пользователей, редактируйте роли и удаляйте учетные записи.",
        fullName: "Полное имя",
        defaultRole: "Роль по умолчанию",
        creating: "Создание...",
        createUser: "Создать пользователя",
        defaultPasswordForNewUsers: "Пароль по умолчанию для новых пользователей:",
        roles: "Роли",
        actions: "Действия",
        noRolesYet: "Ролей пока нет",
        selectRoles: "Выбрать роли",
        noRolesFound: "Роли не найдены.",
        noMethodPermissions: "Нет разрешений на методы",
        methodPermissions: "Разрешения на методы",
        noMethodsFound: "Методы не найдены.",
        loadingUsers: "Загрузка пользователей...",
        noUsersFound: "Пользователи не найдены.",
        eventLogTitle: "Журнал событий",
        eventLogSubtitle: "Просматривайте активность процесса с фильтрами, сортировкой и поиском.",
        searchEventsPlaceholder: "Поиск по деталям, исполнителю, действию...",
        allEntities: "Все сущности",
        allActions: "Все действия",
        newestFirst: "Сначала новые",
        oldestFirst: "Сначала старые",
        applyFilters: "Применить фильтры",
        loading: "Загрузка...",
        showingCount: "Показано {{shown}} из {{total}}",
        collapseEventLog: "Свернуть журнал событий до 10 записей",
        expandEventLog: "Развернуть журнал событий полностью",
        showTop10: "Показать 10",
        showFullList: "Показать всё",
        timestamp: "Время",
        actor: "Исполнитель",
        entity: "Сущность",
        action: "Действие",
        details: "Детали",
        system: "Система",
        loadingEvents: "Загрузка событий...",
        noEventsForFilters: "По текущим фильтрам событий не найдено.",
        placeholders: {
          username: "напр. lab.tech",
          fullName: "напр. Иван Петров",
          email: "напр. user@company.com",
        },
        aria: {
          changeUsername: "Изменить имя пользователя для {{username}}",
          changeFullName: "Изменить полное имя для {{username}}",
          changeEmail: "Изменить email для {{username}}",
        },
        event: {
          entities: {
            sample: "образец",
            planned_analysis: "план_анализ",
            conflict: "конфликт",
            user: "пользователь",
          },
          actions: {
            created: "создано",
            updated: "обновлено",
            delete: "удалено",
            deleted: "удалено",
            status_change: "смена_статуса",
            operator_assigned: "оператор_назначен",
            operator_unassigned: "оператор_снят",
            role_changed: "роль_изменена",
            method_permissions_changed: "разрешения_методов_изменены",
            password_reset_requested: "запрошен_сброс_пароля",
            password_reset_completed: "сброс_пароля_завершен",
            password_changed: "пароль_изменен",
          },
          details: {
            self_service: "Самостоятельно",
            email_flow: "Через email",
            keys: {
              username: "Имя пользователя",
              email: "Email",
              roles: "Роли",
              methods: "Разрешения методов",
              status: "Статус",
              well_id: "ID скважины",
              horizon: "Горизонт",
              sampling_date: "Дата отбора",
              arrival_date: "Дата поступления",
              storage_location: "Место хранения",
              assigned_to: "Назначено",
              sample: "Образец",
              method: "Метод",
              assignees: "Назначенные",
              target: "Цель",
              resolution_note: "Комментарий урегулирования",
            },
          },
        },
        dialogs: {
          to: "на",
          confirm: "Подтвердить",
          updateUsername: {
            title: "Изменить имя пользователя",
            description: "Изменение имени пользователя для",
            save: "Сохранить имя пользователя",
          },
          confirmUsername: {
            title: "Подтвердите изменение имени пользователя",
            description: "Изменить имя пользователя с",
          },
          updateFullName: {
            title: "Изменить полное имя",
            description: "Изменение полного имени для",
            save: "Сохранить полное имя",
          },
          confirmFullName: {
            title: "Подтвердите изменение полного имени",
            description: "Изменить полное имя с",
          },
          updateEmail: {
            title: "Изменить email пользователя",
            description: "Изменение email для",
            save: "Сохранить email",
          },
          confirmEmail: {
            title: "Подтвердите изменение email",
            description: "Изменить",
          },
        },
      },
    },
    settings: {
      title: "Настройки",
      systemStatus: "Состояние системы",
      subtitle: "Фронтенд пока использует мок-данные; бэкенд используется только для health-проверки.",
      backendReachable: "Бэкенд доступен",
      unexpectedResponse: "Неожиданный ответ",
      failedBackend: "Не удалось связаться с бэкендом",
      checking: "Проверка...",
      healthy: "Доступен",
      unavailable: "Недоступен",
      idle: "Ожидание",
      retry: "Повторить",
    },
    samplesPage: {
      title: "Реестр образцов",
      subtitle: "Все уникальные образцы и статусы анализов.",
      loading: "Загрузка образцов...",
      empty: "Образцов пока нет.",
      common: {
        reset: "Сброс",
        cancel: "Отмена",
        apply: "Применить",
      },
      filters: {
        sampleId: "Фильтр по ID образца",
        wellId: "Фильтр по ID скважины",
        samplingDate: "Фильтр по дате отбора",
        arrivalDate: "Фильтр по дате поступления",
        operator: "Фильтр по оператору",
      },
      methods: {
        button: "Методы",
        title: "Фильтр методов",
        chooseMethod: "Выберите метод",
        any: "Любой",
        done: "Выполнен",
        notDone: "Не выполнен",
      },
      sort: {
        button: "Сортировка",
        order: "Порядок сортировки",
        by: "Сортировать по",
        chooseField: "Выберите поле",
        ascending: "По возрастанию",
        descending: "По убыванию",
      },
      columns: {
        sampleId: "ID образца",
        wellId: "ID скважины",
        ngdu: "НГДУ",
        wellNumber: "Номер скважины",
        shop: "Цех",
        field: "Месторождение",
        injectionWell: "Нагнетательная скважина",
        horizon: "Горизонт",
        samplingDate: "Дата отбора",
        arrivalDate: "Дата поступления",
        storageLocation: "Место хранения",
        warehouseStatus: "Статус склада",
        labStatus: "Статус лаборатории",
        adminStatus: "Статус админа",
        analyses: "Анализы",
        method: "Метод",
        done: "Выполнено",
        operators: "Операторы",
      },
      values: {
        yes: "Да",
        no: "Нет",
        na: "Н/Д",
        unassigned: "Не назначен",
      },
    },
    actionsPage: {
      title: "Реестр воздействий",
      subtitle: "Таблица технологических воздействий (мок-данные).",
      common: {
        reset: "Сброс",
        cancel: "Отмена",
        apply: "Применить",
      },
      filters: {
        wellId: "Фильтр по ID скважины",
        actionId: "Фильтр по ID воздействия",
        startDate: "Фильтр по дате начала",
        endDate: "Фильтр по дате окончания",
        actionType: "Фильтр по типу воздействия",
      },
      sort: {
        button: "Сортировка",
        order: "Порядок сортировки",
        by: "Сортировать по",
        chooseField: "Выберите поле",
        ascending: "По возрастанию",
        descending: "По убыванию",
      },
      columns: {
        wellId: "ID скважины",
        actionId: "ID воздействия",
        startDate: "Дата начала",
        endDate: "Дата окончания",
        success: "Успешность",
        actionType: "Тип воздействия",
        incrementalOil: "Фактический прирост добычи нефти за год внедрения, т",
        avgDailyRate: "Фактический среднесуточный дебит до воздействия, т/сут",
      },
    },
    board: {
      roleView: {
        warehouse_worker: "Склад: образцы и хранение",
        lab_operator: "Лаборатория: анализы",
        action_supervision: "Контроль воздействий",
        admin: "Админ",
      },
      sampleTrackingBoard: "Доска отслеживания образцов",
      samplesAcrossStages: "{{count}} образцов на {{stages}} этапах",
      methods: "Методы",
      filterVisibility: "Видимость фильтров",
      changesSaved: "Изменения сохранены",
      filterVisibilityUpdated: "Видимость фильтров обновлена.",
      failedUpdateFilters: "Не удалось обновить фильтры",
      failedLoadData: "Не удалось загрузить данные",
      sort: "Сортировка",
      sortOrder: "Порядок сортировки",
      reset: "Сброс",
      sortBy: "Сортировать по",
      chooseField: "Выберите поле",
      sampleId: "ID образца",
      samplingDate: "Дата отбора",
      methodsDone: "Выполнено методов",
      ascending: "По возрастанию",
      descending: "По убыванию",
      cancel: "Отмена",
      apply: "Применить",
      showAssignedOnly: "Показывать только назначенные мне",
      showIncompleteOnly: "Показывать только незавершенные",
      undo: "Отменить",
      syncing: "Синхронизация...",
      refresh: "Обновить",
      addConflict: "Добавить конфликт",
      newSample: "Новый образец",
      newSampleDialog: {
        title: "Создать новый образец",
        description: "Минимальные поля карточки образца. Сохраняется только локально.",
        sampleId: "ID образца",
        wellId: "ID скважины",
        horizon: "Горизонт",
        samplingDate: "Дата отбора",
        arrivalDate: "Дата поступления",
        pickDate: "Выберите дату",
        storageLocation: "Место хранения",
        create: "Создать",
        errors: {
          required: "Заполните все обязательные поля",
          futureDate: "Дата отбора не может быть в будущем",
          futureArrivalDate: "Дата поступления не может быть в будущем",
          arrivalBeforeSampling: "Дата поступления не может быть раньше даты отбора",
          duplicateSampleId: "ID образца уже существует",
          fillStorageParts: "Заполните Холодильник, Ячейку и Место",
          invalidStorageFormat: "Место хранения должно быть в формате: \"Fridge {A1} · Bin {B2} · Place {C3}\"",
        },
      },
      notifications: {
        methodAssignmentTitle: "Назначение метода",
        methodUnassignmentTitle: "Снятие назначения метода",
        methodAssigned: "{{method}} для {{sampleId}} назначен вам.",
        methodAssignedBy: "{{method}} для {{sampleId}} назначен вам пользователем {{actor}}.",
        methodUnassigned: "{{method}} для {{sampleId}} снят с вас.",
        methodUnassignedBy: "{{method}} для {{sampleId}} снят с вас пользователем {{actor}}.",
        newPlannedSampleTitle: "Новый запланированный образец",
        newPlannedSampleDesc: "{{sampleId}} появился в \"Запланировано\" без использования кнопки \"Новый образец\".",
        returnedForAnalysisTitle: "Возврат на анализ",
        returnedToWarehouseDesc: "{{sampleId}} возвращен на склад администратором.",
        uploadedBatchTitle: "Загруженная партия",
        uploadedBatchDesc: "{{sampleId}} находится в колонке \"Загруженная партия\".",
        conflictDetectedTitle: "Обнаружен конфликт",
        conflictDetectedDesc: "{{sampleId}} находится в колонке \"Конфликты\".",
        issueReportedTitle: "Зафиксирована проблема",
        issueReportedDesc: "{{sampleId}} находится в колонке \"Проблемы\".",
        needsAttentionTitle: "Требует внимания",
        needsAttentionDesc: "{{sampleId}} находится в колонке \"Требует внимания\".",
        newPlannedAnalysisTitle: "Новый запланированный анализ",
        newPlannedAnalysisDesc: "{{sampleId}} находится в колонке \"Запланировано\".",
        returnedToLabDesc: "{{sampleId}} возвращен в лабораторию администратором.",
      },
      loadingBoard: "Загрузка доски...",
      noItemsYet: "Пока нет записей. Создайте образец или анализ.",
      columns: {
        planned: "Запланировано",
        awaiting_arrival: "Ожидание поступления",
        stored: "Хранится",
        issues: "Проблемы",
        in_progress: "В работе",
        needs_attention: "Требует внимания",
        completed: "Завершено",
        uploaded_batch: "Загруженная партия",
        conflicts: "Конфликты",
        deleted: "Удалено",
      },
      columnsShort: {
        planned: "План",
        awaiting_arrival: "Ожидание",
        in_progress: "В работе",
        needs_attention: "Внимание",
        completed: "Завершено",
        stored: "Хранится",
        issues: "Проблемы",
        conflicts: "Конфликты",
        uploaded_batch: "Партия",
        deleted: "Удалено",
      },
      card: {
        sample: "Образец",
        analysis: "Анализ",
        conflictStatus: "Статус конфликта",
        uploadedBatch: "Загруженная партия",
        batch: "Партия",
        conflict: "Конфликт",
        conflicts: "Конфликты",
        resolved: "Урегулирован",
        stored: "Хранится",
        sampling: "Отбор",
        arrival: "Поступление",
        well: "Скважина",
        horizon: "Горизонт",
        reason: "Причина",
        returnNote: "Комментарий возврата",
        issue: "Проблема",
        fridge: "Холодильник",
        bin: "Ячейка",
        place: "Место",
        done: "Готово",
        delete: "Удалить",
        restore: "Восстановить",
        storedAsNotResolved: "Сохранить как неурегулированное",
        returnForAnalysis: "Вернуть на анализ",
        failed: "Ошибка",
        unassigned: "Не назначен",
      },
      cardShort: {
        well: "Скв.",
        horizon: "Гор.",
        fridge: "Холод.",
        bin: "Яч.",
        place: "Место",
        unassigned: "Не назн.",
      },
      actionsShort: {
        sort: "Сорт.",
        filterVisibility: "Фильтры",
        applyFilters: "Применить",
        resolveConflict: "Урегулировать",
        storedAsNotResolved: "Не урегулировано",
        returnForAnalysis: "Вернуть",
      },
    },
    panel: {
      sampleStatus: "Статус образца",
      analysisStatus: "Статус анализа",
      latestIssue: "Последняя проблема",
      latestReturnNote: "Последний комментарий возврата",
      timeUnavailable: "Время недоступно",
      assignedTo: "Назначено",
      addAssignee: "Добавить исполнителя",
      samplingDate: "Дата отбора",
      arrivalDate: "Дата поступления",
      storage: "Хранение",
      resolutionNote: "Комментарий урегулирования",
      systemNote: "Системная запись",
      issueAndReturnNotes: "Проблемы и комментарии возврата",
      issue: "Проблема",
      returnNote: "Комментарий возврата",
      comments: "Комментарии",
      noCommentsYet: "Комментариев пока нет.",
      commentAs: "Комментировать как",
      unknown: "Неизвестно",
      writeComment: "Введите комментарий...",
      addComment: "Добавить комментарий",
      assignOperatorToMethod: "Назначить оператора на метод",
      selectMethod: "Выберите метод",
      assignToLabOperator: "Назначить лаборанту",
      noOperatorsForMethod: "Нет операторов с правом на этот метод.",
      methodRequired: "Требуется выбрать метод",
      selectOperator: "Выберите оператора для назначения",
      labAssignOnlySelf: "Лаборант может назначить только себя",
      noEligibleOperators: "Нет подходящих операторов для этого метода",
      assignOperator: "Назначить оператора",
      addAnalysisAdmin: "Добавить анализ (Админ)",
      analysisPlaceholder: "Напр., ЯМР",
      assignOptional: "Назначить (необязательно)",
      analysisTypeRequired: "Требуется тип анализа",
      addAnalysis: "Добавить анализ",
      resolveConflict: "Урегулировать конфликт",
      resolutionNoteOptional: "Комментарий урегулирования",
      markResolved: "Отметить урегулированным",
      old: "СТАРОЕ",
      new: "НОВОЕ",
    },
  },
};

type I18nContextType = {
  language: Language;
  setLanguage: (lang: Language) => void;
  t: (key: string, vars?: Record<string, string | number>) => string;
};

const I18nContext = createContext<I18nContextType | undefined>(undefined);

function getInitialLanguage(): Language {
  if (typeof window === "undefined") return "en";
  const stored = localStorage.getItem(STORAGE_KEY);
  if (stored === "en" || stored === "ru") return stored;
  return navigator.language.toLowerCase().startsWith("ru") ? "ru" : "en";
}

function resolveKey(dict: Record<string, DictValue>, key: string): string | null {
  const parts = key.split(".");
  let current: DictValue | undefined = dict;
  for (const part of parts) {
    if (!current || typeof current === "string") return null;
    current = current[part];
  }
  return typeof current === "string" ? current : null;
}

function interpolate(text: string, vars?: Record<string, string | number>) {
  if (!vars) return text;
  return text.replace(/\{\{(\w+)\}\}/g, (_, name: string) => String(vars[name] ?? ""));
}

export function I18nProvider({ children }: { children: React.ReactNode }) {
  const [language, setLanguageState] = useState<Language>(getInitialLanguage);

  const setLanguage = (lang: Language) => {
    setLanguageState(lang);
    if (typeof window !== "undefined") {
      localStorage.setItem(STORAGE_KEY, lang);
    }
  };

  const t = (key: string, vars?: Record<string, string | number>) => {
    const selected = resources[language];
    const fallback = resources.en;
    const hit = resolveKey(selected, key) ?? resolveKey(fallback, key) ?? key;
    return interpolate(hit, vars);
  };

  const value = useMemo(() => ({ language, setLanguage, t }), [language]);
  return <I18nContext.Provider value={value}>{children}</I18nContext.Provider>;
}

export function useI18n() {
  const ctx = useContext(I18nContext);
  if (!ctx) throw new Error("useI18n must be used within I18nProvider");
  return ctx;
}
